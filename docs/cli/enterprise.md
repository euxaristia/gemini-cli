...
2. User Settings (`~/.pollux/settings.json`)
3. Workspace Settings (`<project>/.pollux/settings.json`)
...
- **System defaults `system-defaults.json`:**
  ...
  ```json
  {
    ...
    "context": {
      "includeDirectories": ["/etc/pollux-cli/common-context"]
    }
  }
  ```

- **User `settings.json` (`~/.pollux/settings.json`):**
...
- **Workspace `settings.json` (`<project>/.pollux/settings.json`):**
...
- **System overrides `settings.json`:**
  ```json
  {
    ...
    "context": {
      "includeDirectories": ["/etc/pollux-cli/global-context"]
    }
  }
  ```
...
- **Final merged configuration:**
  ```json
  {
    ...
    "context": {
      "includeDirectories": [
        "/etc/pollux-cli/common-context",
        "~/gemini-context",
        "./project-context",
        "/etc/pollux-cli/global-context"
      ]
    }
  }
  ```
...
- **Location**:
  - **Linux**: `/etc/pollux-cli/settings.json`
  - **Windows**: `C:\ProgramData\pollux-cli\settings.json`
  - **macOS**: `/Library/Application Support/GeminiCli/settings.json`
  - The path can be overridden using the `POLLUX_CLI_SYSTEM_SETTINGS_PATH`
...
While the `POLLUX_CLI_SYSTEM_SETTINGS_PATH` environment variable provides
...
export POLLUX_CLI_SYSTEM_SETTINGS_PATH="/etc/pollux-cli/settings.json"
...
By deploying this script, the `POLLUX_CLI_SYSTEM_SETTINGS_PATH` is set within
...
```powershell
Add-Content -Path $PROFILE -Value '$env:POLLUX_CLI_SYSTEM_SETTINGS_PATH="C:\ProgramData\pollux-cli\settings.json"'
```
...
By default, Gemini CLI stores configuration and history in `~/.pollux`. You can
use the `POLLUX_CLI_HOME` environment variable to point to a unique directory
for a specific user or job. The CLI will create a `.pollux` folder inside the
specified path.
...
export POLLUX_CLI_HOME="/tmp/gemini-job-123"
gemini
```
...
```powershell
# Isolate state for a specific job
$env:POLLUX_CLI_HOME="C:\temp\gemini-job-123"
gemini
```
