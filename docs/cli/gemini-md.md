# Provide context with POLLUX.md files

Context files, which use the default name `POLLUX.md`, are a powerful feature
...
1.  **Global context file:**
    - **Location:** `~/.pollux/POLLUX.md` (in your user home directory).
    - **Scope:** Provides default instructions for all your projects.

2.  **Environment and workspace context files:**
    - **Location:** The CLI searches for `POLLUX.md` files in your configured
...
3.  **Just-in-time (JIT) context files:**
    - **Location:** When a tool accesses a file or directory, the CLI
      automatically scans for `POLLUX.md` files in that directory and its
...
### Example `POLLUX.md` file

Here is an example of what you can include in a `POLLUX.md` file at the root of
...
- **`/memory reload`**: Forces a re-scan and reload of all `POLLUX.md` files
  from all configured locations.
- **`/memory add <text>`**: Appends your text to your global
  `~/.pollux/POLLUX.md` file. This lets you add persistent memories on the fly.

## Modularize context with imports

You can break down large `POLLUX.md` files into smaller, more manageable
...
**Example `POLLUX.md` with imports:**
...
While `POLLUX.md` is the default filename, you can configure this in your
...
{
  "context": {
    "fileName": ["AGENTS.md", "CONTEXT.md", "POLLUX.md"]
  }
}
```
