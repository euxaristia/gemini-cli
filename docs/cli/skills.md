...
Unlike general context files ([`POLLUX.md`](./gemini-md.md)), which provide
...
1.  **Workspace Skills**: Located in `.pollux/skills/` or the `.agents/skills/`
    alias. Workspace skills are typically committed to version control and
    shared with the team.
2.  **User Skills**: Located in `~/.pollux/skills/` or the `~/.agents/skills/`
...
precedence over the `.pollux/skills/` directory. This generic alias provides an
...
# Discovers skills (SKILL.md or */SKILL.md) and creates symlinks in ~/.pollux/skills
# (or ~/.agents/skills)
gemini skills link /path/to/my-skills-repo

# Link to the workspace scope (.pollux/skills or .agents/skills)
...
# Uses the user scope by default (~/.pollux/skills or ~/.agents/skills)
...
# Install to the workspace scope (.pollux/skills or .agents/skills)
