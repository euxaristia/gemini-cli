...
via a `.pollux/.env` file. See
[Persisting Environment Variables](../get-started/authentication.md#persisting-environment-variables).

- Use the project default path (`.pollux/system.md`):
  - `GEMINI_SYSTEM_MD=true` or `GEMINI_SYSTEM_MD=1`
  - The CLI reads `./.pollux/system.md` (relative to your current project
    directory).
...
- Persist for a project using `.pollux/.env`:
  - Create `.pollux/system.md`, then add to `.pollux/.env`:
    - `GEMINI_SYSTEM_MD=1`
...
## Best practices: SYSTEM.md vs POLLUX.md
...
POLLUX.md focused on high‑level guidance and project specifics.

## Troubleshooting

- Error: `missing system prompt file '…'`
  - Ensure the referenced path exists and is readable.
  - For `GEMINI_SYSTEM_MD=1|true`, create `./.pollux/system.md` in your project.
- Override not taking effect
  - Confirm the variable is loaded (use `.pollux/.env` or export in your shell).
