...
- **Sub-commands:**
  ...
  - **`reload`** (alias: `refresh`):
    - **Description:** Rescans agent directories (`~/.pollux/agents` and
      `.pollux/agents`) and reloads the registry.
    - **Usage:** `/agents reload`
  - **`enable`**:
...
- **Sub-commands:**
  ...
  - **`save <tag>`**
    ...
    - **Details on checkpoint location:** The default locations for saved chat
      checkpoints are:
      - Linux/macOS: `~/.pollux/tmp/<project_hash>/`
      - Windows: `C:\Users\<YourUsername>\.pollux\tmp\<project_hash>\`
...
- **Sub-commands:**
  - **`reload`**:
    - **Description:** Reload custom command definitions from all sources
      (user-level `~/.pollux/commands/`, project-level
      `<project>/.pollux/commands/`, MCP prompts, and extensions). Use this to
...
- **Description:** To help users easily create a `POLLUX.md` file, this command
...
- **Description:** Manage the AI's instructional context (hierarchical memory
  loaded from `POLLUX.md` files).
- **Sub-commands:**
  - **`add`**:
    - **Description:** Adds the following text to the AI's memory. Usage:
      `/memory add <text to remember>`
  - **`list`**:
    - **Description:** Lists the paths of the POLLUX.md files in use for
      hierarchical memory.
  - **`refresh`**:
    - **Description:** Reload the hierarchical instructional memory from all
      `POLLUX.md` files found in the configured locations (global,
      project/ancestors, and sub-directories). This command updates the model
      with the latest `POLLUX.md` content.
  - **`show`**:
    - **Description:** Display the full, concatenated content of the current
      hierarchical memory that has been loaded from all `POLLUX.md` files. This
      lets you inspect the instructional context being provided to the Gemini
      model.
  - **Note:** For more details on how `POLLUX.md` files contribute to
...
- **Details:** This command provides a user-friendly interface for changing
  ...
  equivalent to manually editing the `.pollux/settings.json` file, but with
...
- **Features:**
  ...
  - **Persistent setting:** Vim mode preference is saved to
    `~/.pollux/settings.json` and restored between sessions
