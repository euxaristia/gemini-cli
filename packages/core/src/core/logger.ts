...
export class Logger {
  private polluxDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.polluxDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.polluxDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.polluxDir, { recursive: true });
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.polluxDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.polluxDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.polluxDir) {
      debugLogger.error(
