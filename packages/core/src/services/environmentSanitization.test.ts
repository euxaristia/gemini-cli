...
describe('sanitizeEnvironment', () => {
  ...
  it('should allow variables prefixed with POLLUX_CLI_', () => {
    const env = {
      POLLUX_CLI_FOO: 'bar',
      POLLUX_CLI_BAZ: 'qux',
    };
  ...
  describe('value-first security: secret values must be caught even for allowed variable names', () => {
    ...
    it('should NOT redact POLLUX_CLI_ variables even if their value looks like a secret (fully trusted)', () => {
      const env = {
        POLLUX_CLI_INTERNAL: 'ghp_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
      };
      const sanitized = sanitizeEnvironment(env, EMPTY_OPTIONS);
      expect(sanitized).toEqual(env);
    });
