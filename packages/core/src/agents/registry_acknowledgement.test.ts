...
describe('AgentRegistry Acknowledgement', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'pollux-cli-test-'));

    // Override POLLUX_CLI_HOME to point to the temp directory
    originalGeminiCliHome = process.env['POLLUX_CLI_HOME'];
    process.env['POLLUX_CLI_HOME'] = tempDir;
  ...
  afterEach(async () => {
    ...
    if (originalGeminiCliHome) {
      process.env['POLLUX_CLI_HOME'] = originalGeminiCliHome;
    } else {
