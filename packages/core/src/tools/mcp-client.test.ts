...
describe('mcp-client', () => {
  ...
  describe('createTransport', () => {
    ...
    it('should exclude extension settings with undefined values from environment', async () => {
      ...
      const callArgs = mockedTransport.mock.calls[0][0];
      expect(callArgs.env).toBeDefined();
      expect(callArgs.env!['POLLUX_CLI_EXT_VAR']).toBeUndefined();
    });
    ...
    it('should include extension settings with defined values in environment', async () => {
      ...
      const callArgs = mockedTransport.mock.calls[0][0];
      expect(callArgs.env).toBeDefined();
      expect(callArgs.env!['POLLUX_CLI_EXT_VAR']).toBe('defined-value');
    });
    ...
    it('should resolve environment variables in mcpServerConfig.env using extension settings', async () => {
      ...
      await createTransport(
        'test-server',
        {
          command: 'test-command',
          env: {
            RESOLVED_VAR: '$POLLUX_CLI_EXT_VAR',
          },
          extension: {
            name: 'test-ext',
            resolvedSettings: [
      ...
      const callArgs = mockedTransport.mock.calls[0][0];
      expect(callArgs.env).toBeDefined();
      expect(callArgs.env!['POLLUX_CLI_EXT_VAR']).toBe('ext-value');
