...
import { POLLUX_CLI_COMPANION_EXTENSION_NAME } from './constants.js';
...
class VsCodeInstaller implements IdeInstaller {
  ...
  async install(): Promise<InstallResult> {
    ...
    try {
      ...
      return {
        success: false,
        message: `Failed to install ${this.ideInfo.displayName} companion extension. Please try installing '${POLLUX_CLI_COMPANION_EXTENSION_NAME}' manually from the ${this.ideInfo.displayName} extension marketplace.`,
      };
    }
  }
}
...
class PositronInstaller implements IdeInstaller {
  ...
  async install(): Promise<InstallResult> {
    ...
    if (!commandPath) {
      return {
        success: false,
        message: `${this.ideInfo.displayName} CLI not found. Please ensure 'positron' is in your system's PATH. For help, see https://positron.posit.co/add-to-path.html. You can also install the '${POLLUX_CLI_COMPANION_EXTENSION_NAME}' extension manually from the VS Code marketplace / Open VSX registry.`,
      };
    }
