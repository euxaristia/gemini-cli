...
describe('ide-connection-utils', () => {
  beforeEach(() => {
    ...
    vi.stubEnv('POLLUX_CLI_IDE_WORKSPACE_PATH', '/test/workspace');
    vi.stubEnv('POLLUX_CLI_IDE_SERVER_PORT', '');
    vi.stubEnv('POLLUX_CLI_IDE_SERVER_STDIO_COMMAND', '');
    vi.stubEnv('POLLUX_CLI_IDE_SERVER_STDIO_ARGS', '');
    vi.stubEnv('POLLUX_CLI_IDE_AUTH_TOKEN', '');
  ...
  describe('getConnectionConfigFromFile', () => {
    ...
    it('should prioritize the config matching the port from the environment variable', async () => {
      vi.stubEnv('POLLUX_CLI_IDE_SERVER_PORT', '2222');
      const config1 = { port: '1111', workspacePath: '/test/workspace' };
    ...
    it('should match env port string to a number port in the config', async () => {
      vi.stubEnv('POLLUX_CLI_IDE_SERVER_PORT', '3333');
      const config1 = { port: 1111, workspacePath: '/test/workspace' };
