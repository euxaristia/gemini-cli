...
import { debugLogger, type Config } from '@euxaristia/pollux-cli-core';
...
async function startOrJoinDevTools(
  ...
  const mod = await import('@euxaristia/pollux-cli-devtools');
...
export async function setupInitialActivityLogger(config: Config) {
  const target = process.env['POLLUX_CLI_ACTIVITY_LOG_TARGET'];
