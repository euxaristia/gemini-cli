...
describe('skillUtils', () => {
  ...
  describe('linkSkill', () => {
    ...
    itif(process.platform !== 'win32')(
      ...
      async () => {
        ...
        const linkedPath = path.join(tempDir, '.pollux/skills', 'test-skill');
    ...
    itif(process.platform !== 'win32')(
      ...
      async () => {
        ...
        const targetDir = path.join(tempDir, '.pollux/skills');
  ...
  it('should successfully install from a .skill file', async () => {
    ...
    const installedPath = path.join(tempDir, '.pollux/skills', 'weather-skill');
  ...
  it('should successfully install from a local directory', async () => {
    ...
    const installedPath = path.join(tempDir, '.pollux/skills', 'test-skill');
    const installedExists = await fs.stat(installedPath).catch(() => null);
    expect(installedExists?.isDirectory()).toBe(true);
  });
  ...
  describe('uninstallSkill', () => {
    it('should successfully uninstall an existing skill', async () => {
      const skillsDir = path.join(tempDir, '.pollux/skills');
