...
import { TestRig, POLLUX_DIR } from './test-helper.js';
...
describe('User Policy Regression Repro', () => {
  ...
  it('should respect policies in ~/.pollux/policies/allowed-tools.toml', async () => {
    rig.setup('user-policy-test', {
      fakeResponsesPath: join(import.meta.dirname, 'user-policy.responses'),
    });

    // Create ~/.pollux/policies/allowed-tools.toml
    const userPoliciesDir = join(rig.homeDir!, POLLUX_DIR, 'policies');
